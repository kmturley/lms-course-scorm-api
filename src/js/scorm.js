...
//            console.log(SCOState);
            commitState = Object.assign({}, SCOState);	//make a copy of the object, so we can unset the session_time if we're not finished
            if (parameter != 'finish') {
...
//                	return match = /^[0-9a-z]?$/i.test(parameter);                  //Single character or digit			//Commented out because Articulate may send the whole word/sentence (more convenient)
        ...
        case 'CMIIdentifier':
            return match = /^[\s\w\-]{1,1024}$/i.test(parameter);                              //A string (allow white space and dash)
            break;
